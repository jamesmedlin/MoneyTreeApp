...
const SavedVideos = ({ route, navigation }: Props) => {
  ...
  const mapSavedVideos = async () => {
    ...
    if (user.customData.savedAds.length != 0) {
      let result = await user.functions.getSavedAdsFull(
        user.customData.savedAds
      );
      setSavedAds(result);
    } else {
    }
                ...
                >
                  <View style={styles.unsaveContainer} key={ad.id}>
                    <TouchableOpacity
                      ...
                      key={ad.id}
                    >
                      <Text style={styles.unsaveText} key={ad.id}>
                        Unsave
                      </Text>
                    </TouchableOpacity>
                  </View>
      ...
      return (
        <View style={{ width: width - 24, alignItems: "center" }}>
          <Text style={styles.text}>
            Save ad websites that interest you in the explore section.
          </Text>
        </View>
      );
    }
    ...
    <View>
      ...
      >
        <View style={styles.innerContainer}>
          <Space.V s={10} />
          {(startedChecking && !checkedSaved) || unsaveLoading ? (
            <ActivityIndicator size="small" />
          ) : null}
          <Space.V s={10} />
          <View style={styles.blah}>{checkedSaved && renderSavedVideos()}</View>
        </View>
        {webpage && selectedAd && (
          <Modal animationType="slide" presentationStyle="pageSheet">
            <View style={styles.view}>
              ...
              >
                <Text style={styles.backButtonText}>Go Back To Saved Ads</Text>
              </TouchableOpacity>
              <WebView
