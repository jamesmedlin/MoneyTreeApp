    ...
    if (permission) {
      let advert = await Geolocation.getCurrentPosition(
        async (position) => {
          ...
          let result = await user.functions.getAdvertisement(
            latitude,
            longitude
          );
          if (result) {
    ...
    <View style={styles.innerContainer}>
      {webpage && (
        <Modal animationType="slide" presentationStyle="pageSheet">
          <View style={styles.view}>
            ...
            >
              <Text style={styles.backButtonText}>Go Back To Video</Text>
            </TouchableOpacity>
            <WebView source={{ uri: ad.website }} startInLoadingState={true} />
          </View>
        </Modal>
      )}
      {ad ? (
        ...
        <Modal animationType="slide" presentationStyle="pageSheet">
          <View style={styles.modal}>
            ...
            <View style={styles.choicesContainer}>
              {selectedAnswer == ad.quiz[0] ? (
                <TouchableOpacity
                  onPress={() => setAnswer(ad.quiz[0])}
                  style={styles.choicesSelected}
                >
                  <Text
                    style={styles.choicesTextSelected}
                  >{`${ad.quiz[0]}`}</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
                ...
                >
                  <Text style={styles.choicesText}>{`${ad.quiz[0]}`}</Text>
                </TouchableOpacity>
              )}
              {selectedAnswer == ad.quiz[1] ? (
                <TouchableOpacity
                  onPress={() => setAnswer(ad.quiz[1])}
                  style={styles.choicesSelected}
                >
                  <Text
                    style={styles.choicesTextSelected}
                ...
                <TouchableOpacity
                  ...
                  style={styles.choices}
                >
                  <Text style={styles.choicesText}>{`${ad.quiz[1]}`}</Text>
                </TouchableOpacity>
              )}
              {selectedAnswer == ad.quiz[2] ? (
                <TouchableOpacity
                  onPress={() => setAnswer(ad.quiz[2])}
                  style={styles.choicesSelected}
                >
                  <Text
                    style={styles.choicesTextSelected}
                ...
                <TouchableOpacity
                  ...
                  style={styles.choices}
                >
                  <Text style={styles.choicesText}>{`${ad.quiz[2]}`}</Text>
                </TouchableOpacity>
              )}
              {quizLoading ? (
                <ActivityIndicator
                  size="small"
                  style={styles.activityIndicatorSubmit}
                />
              ) : (
                <TouchableOpacity
      ...
      {ad && (
        <View style={styles.videoButtons}>
          ...
          >
            <Text style={styles.playPauseText}>Play/Pause</Text>
          </TouchableOpacity>
          <View style={styles.optionButtons}>
            {saveLoading && (
              <ActivityIndicator
                size="small"
                style={styles.activityIndicatorSave}
              />
            )}
            {isAlreadySaved != null && !saveLoading && (
              <TouchableOpacity
      ...
      {!ad && !hasStarted && (
        <TouchableOpacity
          ...
          style={styles.startWatchingContainer}
        >
